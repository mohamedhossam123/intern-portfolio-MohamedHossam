...
const useAnimationLoop = (trackRef, targetVelocity, seqWidth, seqHeight, isHovered, hoverSpeed, isVertical, containerWidth) => {
  ...
  useEffect(() => {
    ...
    if (seqSize > 0) {
      // Initialize offset to start from right side (off-screen)
...
export const LogoLoop = memo<LogoLoopProps>(
  ({
    ...
    useAnimationLoop(trackRef, targetVelocity, seqWidth, seqHeight, isHovered, effectiveHoverSpeed, isVertical, containerRef.current?.clientWidth ?? 0);
